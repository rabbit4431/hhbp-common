...
function pushPathCandidate(paths, value) {
  ...
  if (!candidate) return;
  if (/^(https?:\/\/|app:\/\/|plugin:\/\/|mcp:\/\/)/i.test(candidate)) return;
...
function pushFileEvent(events, value, action, diffPreview, patchPreview) {
  ...
  const normalizedDiffPreview = typeof diffPreview === 'string' && diffPreview.trim()
    ? diffPreview.trim() : undefined;
  const normalizedPatchPreview = typeof patchPreview === 'string' && patchPreview.trim()
    ? patchPreview.trim() : undefined;
  if (!events.some(event =>
    event.path === candidate && event.action === action &&
    ...
    if (normalizedDiffPreview) event.diff_preview = normalizedDiffPreview;
    if (normalizedPatchPreview) event.patch_preview = normalizedPatchPreview;
    events.push(event);
  }
}
...
function buildPatchPreviewFromReplacement(oldValue, newValue) {
  ...
  if (beforeLines.length === 0 && afterLines.length === 0) return undefined;
  const lines = ['@@'];
...
function buildDiffPreviewFromPatchPreview(patchPreview) {
  if (typeof patchPreview !== 'string' || !patchPreview.trim()) return undefined;
...
function inferDefaultFileAction(toolName) {
  ...
  if (normalized.includes('write')) return 'create';
  if (normalized.includes('edit')) return 'modify';
...
function actionForFileKey(toolName, key) {
  if (key === 'source_path' || key === 'old_file_path') return 'move';
  if (key === 'destination_path' || key === 'new_file_path') return 'move';
  return inferDefaultFileAction(toolName);
}

function collectFilePaths(value, paths) {
  if (!value) return;
  if (Array.isArray(value)) { for (const entry of value) collectFilePaths(entry, paths); return; }
  if (typeof value === 'string') { pushPathCandidate(paths, value); return; }
  if (typeof value !== 'object') return;
  for (const [key, nested] of Object.entries(value)) {
    if (FILE_PATH_KEYS.has(key)) { collectFilePaths(nested, paths); continue; }
    if (nested && (Array.isArray(nested) || typeof nested === 'object')) collectFilePaths(nested, paths);
  }
}
...
function fileEventDiffPreview(toolName, value, action) {
  ...
  if (action === 'create') return buildCreationPreview(value.content || value.file_text || value.text);
  return undefined;
}
...
function fileEventPatchPreview(value, action) {
  ...
  if (action === 'create') return buildPatchPreviewFromContent(value.content || value.file_text || value.text, '+');
  if (action === 'delete') return buildPatchPreviewFromContent(value.content || value.old_string || value.file_text, '-');
  return undefined;
}
...
function patchPreviewFromGitDiff(repoRoot, pathCandidates) {
  for (const candidate of pathCandidates) {
    const patch = runGit(['diff', '--no-ext-diff', '--no-color', '--unified=1', '--', candidate], repoRoot);
    if (!patch) continue;
...
function enrichFileEventFromWorkingTree(toolName, event) {
  ...
  if (tool.includes('write')) {
    return { ...event, action: tracked ? 'modify' : event.action, diff_preview: diffPreview, patch_preview: patchPreview };
  }
  if (tracked && patchPreview) {
    return { ...event, diff_preview: diffPreview, patch_preview: patchPreview };
  }
  return event;
}
...
function collectFileEvents(toolName, value, events, key = null, parentValue = null) {
  ...
  if (Array.isArray(value)) {
    for (const entry of value) collectFileEvents(toolName, entry, events, key, parentValue);
    return;
  }
  if (typeof value === 'string') {
    if (key && FILE_PATH_KEYS.has(key)) {
      const action = actionForFileKey(toolName, key);
      pushFileEvent(events, value, action,
...
function extractFileEvents(toolName, toolInput) {
  const events = [];
  if (!toolInput || typeof toolInput !== 'object') return events;
...
function summarizeInput(toolName, toolInput, filePaths) {
  ...
  if (toolInput && typeof toolInput === 'object') {
    ...
    for (const [key, value] of Object.entries(toolInput)) {
      ...
      if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') shallow[key] = value;
    }
...
function summarizeOutput(toolOutput) {
  ...
  if (typeof toolOutput === 'string') return truncateSummary(toolOutput);
  if (typeof toolOutput === 'object' && typeof toolOutput.output === 'string') return truncateSummary(toolOutput.output);
...
function buildActivityRow(input, env = process.env) {
  ...
  if (hookEvent && hookEvent !== 'PostToolUse') return null;

  const toolName = String(input?.tool_name || '').trim();
  const sessionId = String(env.HHBP_SESSION_ID || env.CLAUDE_SESSION_ID || '').trim();
  if (!toolName || !sessionId) return null;
