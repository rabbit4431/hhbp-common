...
function hydrateSessionFromPath(sessionPath) {
  ...
  if (!parsed) throw new Error(`Unsupported session file: ${sessionPath}`);
...
function resolveSessionRecord(target, cwd) {
  ...
  if (explicitTarget) {
    if (explicitTarget === 'latest') {
      ...
      if (!latest) throw new Error('No Claude session history found');
      return {
        session: sessionManager.getSessionById(latest.filename, true),
        sourceTarget: { type: 'claude-history', value: 'latest' }
      };
    }
    ...
    if (alias) {
      return {
        ...
        sourceTarget: { type: 'claude-alias', value: explicitTarget }
      };
    }

    const session = sessionManager.getSessionById(explicitTarget, true);
    if (!session) throw new Error(`Claude session not found: ${explicitTarget}`);
    return {
      session,
      sourceTarget: { type: 'claude-history', value: explicitTarget }
    };
  }
  ...
  if (isSessionFileTarget(target, cwd)) {
    return {
      ...
      sourceTarget: { type: 'session-file', value: path.resolve(cwd, target) }
    };
  }
...
function createClaudeHistoryAdapter(options = {}) {
  ...
  return {
    ...
    canOpen(target, context = {}) {
      if (context.adapterId && context.adapterId !== 'claude-history') return false;
      if (context.adapterId === 'claude-history') return true;
