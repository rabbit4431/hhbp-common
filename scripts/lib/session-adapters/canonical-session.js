...
const SESSION_SCHEMA_VERSION = 'hhbp.session.v1';
const SESSION_RECORDING_SCHEMA_VERSION = 'hhbp.session.recording.v1';
const DEFAULT_RECORDING_DIR = path.join(os.homedir(), '.hhbp-session-recordings');
...
function parseContextSeedPaths(context) {
  if (typeof context !== 'string' || context.trim().length === 0) return [];
...
function buildAggregates(workers) {
  const states = workers.reduce((acc, worker) => {
    const state = worker.state || 'unknown';
    acc[state] = (acc[state] || 0) + 1;
    return acc;
  }, {});

  const healths = workers.reduce((acc, worker) => {
    const health = worker.health || 'unknown';
    acc[health] = (acc[health] || 0) + 1;
    return acc;
  }, {});

  return { workerCount: workers.length, states, healths };
}

function validateCanonicalSnapshot(snapshot) {
  if (!isObject(snapshot)) throw new Error('Canonical session snapshot must be an object');
  ...
  if (!isObject(snapshot.session)) throw new Error('Canonical session snapshot requires session to be an object');
  ...
  if (!isObject(snapshot.session.sourceTarget)) throw new Error('Canonical session snapshot requires session.sourceTarget to be an object');
  ...
  if (!Array.isArray(snapshot.workers)) throw new Error('Canonical session snapshot requires workers to be an array');

  snapshot.workers.forEach((worker, index) => {
    if (!isObject(worker)) throw new Error(`Canonical session snapshot requires workers[${index}] to be an object`);
    ...
    if (!isObject(worker.runtime)) throw new Error(`Canonical session snapshot requires workers[${index}].runtime to be an object`);
    ...
    if (!isObject(worker.intent)) throw new Error(`Canonical session snapshot requires workers[${index}].intent to be an object`);
    ...
    if (!isObject(worker.outputs)) throw new Error(`Canonical session snapshot requires workers[${index}].outputs to be an object`);
    ...
    if (!isObject(worker.artifacts)) throw new Error(`Canonical session snapshot requires workers[${index}].artifacts to be an object`);
  });

  if (!isObject(snapshot.aggregates)) throw new Error('Canonical session snapshot requires aggregates to be an object');
  ...
  if (!isObject(snapshot.aggregates.states)) throw new Error('Canonical session snapshot requires aggregates.states to be an object');
  if (!isObject(snapshot.aggregates.healths)) throw new Error('Canonical session snapshot requires aggregates.healths to be an object');
...
function resolveRecordingDir(options = {}) {
  ...
  if (typeof process.env.HHBP_SESSION_RECORDING_DIR === 'string' && process.env.HHBP_SESSION_RECORDING_DIR.length > 0) {
    return path.resolve(process.env.HHBP_SESSION_RECORDING_DIR);
  }
...
function writeFallbackSessionRecording(snapshot, options = {}) {
  ...
  const payload = {
    ...
    history: Array.isArray(existing && existing.history)
      ? (snapshotChanged ? existing.history.concat([{ recordedAt, snapshot }]) : existing.history)
      : [{ recordedAt, snapshot }]
  };

  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  fs.writeFileSync(filePath, JSON.stringify(payload, null, 2) + '\n', 'utf8');
  return { backend: 'json-file', path: filePath, recordedAt };
}

function loadStateStore(options = {}) {
  if (options.stateStore) return options.stateStore;
  ...
  try {
    ...
    if (missingRequestedModule) return null;
    throw error;
  }
}
...
function resolveStateStoreWriter(stateStore) {
  ...
  const candidates = [
    ...
    { owner: stateStore.sessions, fn: stateStore.sessions && stateStore.sessions.persistCanonicalSessionSnapshot },
    { owner: stateStore.sessions, fn: stateStore.sessions && stateStore.sessions.recordCanonicalSessionSnapshot },
  ];
...
function persistCanonicalSnapshot(snapshot, options = {}) {
  validateCanonicalSnapshot(snapshot);
  if (options.persist === false) return { backend: 'skipped', path: null, recordedAt: null };

  const stateStore = loadStateStore(options);
  const writer = resolveStateStoreWriter(stateStore);

  if (stateStore && !writer) return writeFallbackSessionRecording(snapshot, options);
  if (writer) {
    writer(snapshot, { adapterId: snapshot.adapterId, schemaVersion: snapshot.schemaVersion, sessionId: snapshot.session.id });
    return { backend: 'state-store', path: null, recordedAt: null };
  }
...
function deriveClaudeWorkerId(session) {
  if (session.shortId && session.shortId !== 'no-id') return session.shortId;
