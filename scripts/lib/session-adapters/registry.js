...
function normalizeStructuredTarget(target, context = {}) {
  ...
  if (type.length === 0) throw new Error('Structured session targets require a non-empty type');

  const adapterId = target.adapterId || TARGET_TYPE_TO_ADAPTER_ID[type] || context.adapterId || null;
  const nextContext = { ...context, adapterId };

  if (type === 'claude-history' || type === 'claude-alias') {
    return { target: `claude:${value}`, context: nextContext };
  }

  return { target: value, context: nextContext };
}
...
function createAdapterRegistry(options = {}) {
  ...
  return {
    ...
    getAdapter(id) {
      ...
      if (!adapter) throw new Error(`Unknown session adapter: ${id}`);
      return adapter;
    },
    listAdapters() {
    ...
    select(target, context = {}) {
      ...
      if (!adapter) throw new Error(`No session adapter matched target: ${target}`);
      return adapter;
    },
    open(target, context = {}) {
      const normalized = normalizeStructuredTarget(target, context);
