...
const DEFAULT_STATE_STORE_RELATIVE_PATH = path.join('.claude', 'hhbp', 'state.db');
 ...
 * that migrations.js and queries.js can operate without knowing which driver
 * is in use.
 *
 * IMPORTANT: sql.js db.export() implicitly ends any active transaction, so
 * all disk writes are deferred until after the transaction commits.
 */
function wrapSqlJsDatabase(rawDb, dbPath) {
  ...
  return {
    ...
    pragma(pragmaStr) {
      try { rawDb.run(`PRAGMA ${pragmaStr}`); } catch { /* ignore unsupported pragmas */ }
    },
    prepare(sql) {
      return {
        all(...positionalArgs) {
          const stmt = rawDb.prepare(sql);
          if (positionalArgs.length === 1 && typeof positionalArgs[0] !== 'object') stmt.bind([positionalArgs[0]]);
          else if (positionalArgs.length > 1) stmt.bind(positionalArgs);
          const rows = [];
...
async function openDatabase(SQL, dbPath) {
  ...
  try { db.pragma('journal_mode = WAL'); } catch { /* ignore WAL failures for in-memory */ }
  return db;
}
...
async function createStateStore(options = {}) {
  ...
  return {
    dbPath,
    close() { db.close(); },
    getAppliedMigrations() { return getAppliedMigrations(db); },
    validateEntity,
    assertValidEntity,
    ...queryApi,
    _database: db,
