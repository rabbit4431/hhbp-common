...
```bash
node -e "
const _r = (()=>{var e=process.env.CLAUDE_PLUGIN_ROOT;if(e&&e.trim()){var r=e.trim(),p=require('path'),f=require('fs');if(f.existsSync(p.join(r,'common','scripts','lib','utils.js')))return p.join(r,'common');return r}var p=require('path'),f=require('fs'),h=require('os').homedir(),d=p.join(h,'.claude'),q=p.join('scripts','lib','utils.js'),qc=p.join('common','scripts','lib','utils.js');if(f.existsSync(p.join(d,q)))return d;for(var s of [['hhbp-claude-code'],['hhbp-claude-code@hhbp-claude-code'],['marketplace','hhbp-claude-code']]){var l=p.join(d,'plugins',...s);if(f.existsSync(p.join(l,qc)))return p.join(l,'common');if(f.existsSync(p.join(l,q)))return l}try{var b=p.join(d,'plugins','cache');var hc=p.join(b,'hhbp-claude-code');if(f.existsSync(hc)){for(var pn of f.readdirSync(hc,{withFileTypes:true})){if(!pn.isDirectory())continue;var px=p.join(hc,pn.name);for(var v of f.readdirSync(px,{withFileTypes:true})){if(!v.isDirectory())continue;var c=p.join(px,v.name);if(f.existsSync(p.join(c,qc)))return p.join(c,'common');if(f.existsSync(p.join(c,q)))return c}}}for(var o of f.readdirSync(b,{withFileTypes:true})){if(!o.isDirectory())continue;var pd=p.join(b,o.name);for(var pn of f.readdirSync(pd,{withFileTypes:true})){if(!pn.isDirectory())continue;var px=p.join(pd,pn.name);for(var v of f.readdirSync(px,{withFileTypes:true})){if(!v.isDirectory())continue;var c=p.join(px,v.name);if(f.existsSync(p.join(c,q)))return c;if(f.existsSync(p.join(c,qc)))return p.join(c,'common')}}}}catch(x){}return d})();
